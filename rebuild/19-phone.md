```html
<!-- ============================================================
     rv patch — phone instrument, stepper, tab ids, h1s, aria-labels
     Drop inside the rebuild's IIFE. Idempotent: everything re-runs
     from current DOM state on every redraw; nothing is applied twice.
     ============================================================ -->
<script>
...
**1. Phone layout.** A `MutationObserver` on `body` (plus resize and breakpoint listeners) re-runs an apply pass after every redraw of `#hero`. Below 760px the pass moves the freshly rendered desktop instrument into a hidden `.rv-desk` (so its ids, inline handlers and delegated marks stay live for the app) and builds a `#hero.phone` phone instrument in its place; above 760px it unwraps the desktop markup back, so 1440px is untouched. The phone model comes from the page's own `heroData` payload where reachable (guarded reads of `heroData`, `opName`, `monthName`, `partialMonth`), falls back to the `data-aim`/`data-take` marks in the DOM, and — because only one rail is ever open on the desktop instrument — walks the rail picker programmatically to harvest the closed rails before restoring the original one. It renders the original's phone anatomy: sticky bar with count, removable clause chips and an All-filters badge; four collapsible WHEN/WHERE/WHO/WHAT-IT-FORCED sections; a pannable month-chip strip (44px chips, part-months striped, presets for all/this year/last 12 months/last 90 days); the 3×3 spatial zone grid shaded by the same alpha curve as the drawing, plus the two "cannot be drawn" pads; airline and airframe ladders at 52px rows; the specimen, margin and seam carried over from the desktop render; specimen dropped below 479px. Every mark is two-tap: the first tap raises the fixed pill ("<what you pointed at> · take it →"), a second tap on the same mark or the pill commits via the page's own `takeFilter`/`setFilter` (falling back to setting the filter control and firing `change`), and scroll or a tap elsewhere cancels. Months use tap-first, tap-last to take a range into the from/to fields.

**2. Stepper.** `rvFixStepper` re-derives the label on every case render: it reads the pool from the case buttons in `#results` (or the first "of N loaded" sighting), the match total from "of N that match"/`LAST_TOTAL`, and the index from the current `?case=` id, rewriting the label to `idx+1 of loaded` with the "of N that match" clause only when the selection holds more than is loaded — so it reads 1 of 100, 2 of 100, 3 of 100 and never shrinks. The previous arrow gets `disabled` on the first record, the next on the last loaded one. Stepping is tracked by a capture listener on the ‹/› buttons for cases the URL can't resolve.

**3. Smaller ones.** All `vtab-*` ids are renamed to `tab-*`, with every `aria-labelledby`/`aria-controls` reference updated to match. The eight panels that carry a title in the original (`p-fleet`, `p-emerging`, `p-clusters`, `p-structure`, `p-age`, `p-engines`, `p-consequences`, `p-found`) get exactly one `<h1>` each — promoting the panel's own `<h2>` where it is a direct child, inserting the original title otherwise, with CSS so the promoted heading keeps the h2 look. The twelve filter selects get their `aria-label`s back by id (Operator … Airframe age), plus the aim-kind select ("What kind of thing to look for") and the one-day date field ("One date"), with a first-option-based sweep as a safety net for any select the id map misses.

Everything is `rv`/`RV_`-prefixed, re-applies from current state (no double application), and if the phone build ever throws it unwraps back to the desktop instrument rather than leaving a half-rendered page.