```html
<!-- ---- review fixes: drop inside the rebuild's IIFE ----------------------- -->
<script>
(function(){
...
**Fault 1 — rails behind the phone layout.** A `MutationObserver` on the body re-runs `sdrStripRails` after every batch, so each time the rebuild's `drawHero` leaks desktop rails at ≤760px they are removed again; a `matchMedia` change listener strips them the moment the viewport crosses 760px, before the debounced redraw. Nothing phone-side is touched, and above 760px the rails the rebuild renders are left alone.

**Fault 2 — case sheet as a page card.** The patch creates `#case-wrap` (the reference's fixed, scrimmed, `overflow:auto` overlay, `z-index:60`, `overscroll-behavior:contain`) and moves the rebuild's `#case-box` inside it, so the card stops growing the document and the overlay is what scrolls — from viewport top, at every width, down to its last line. Its CSS mirrors the reference geometry, with the action bar made `position:sticky; top:0` inside the overlay so Close and the steppers stay put. Opening is driven by the sheet's own render landing in `#case-box` (the rebuild's `openCase` keeps working untouched), closing by Escape, a scrim click, any button labelled Close, the box being emptied, or Back — with the open state pushed to history only if the rebuild has not already done so. While open, siblings are inert and focus is trapped; on close, focus returns to whatever opened the sheet.

**Fault 3 — unlabelled selects.** The nine missing `aria-label`s are applied with the reference's exact wording, only where the attribute is absent, and re-checked after every redraw so a re-rendered filter row can't lose them — bringing the count to twenty-two without overwriting the thirteen the rebuild already labels.