Two changes, everything else carried through untouched:

1. **Dead inline handlers** — `window.setFilter(field, value)` and `window.setHero(name)` are now defined globally, only when the page hasn't exported its own (re-checked every pass, so an export always wins). `setFilter` narrows by setting the query parameter and letting the page apply it on load — URL, sentence and rows then agree. `setHero` opens the named rail (clicks the page's own toggle first, forces the open class as fallback, then scrolls to it).
2. **Part number** — `fetch`/XHR responses that look like one record (own `part_number`, no `records` array, not a dossier endpoint) are picked up in passing. The case sheet gets its own **Part number** row right beside the Part row, filled from the fetched record, rendered as a `code.cd` that opens the part dossier (keyboard included). When the record has no part number, the row says so in the sheet's own absence words, detected from its other rows. No `<code class="cd">P/N</code>` is invented anywhere it doesn't exist.
...
/* ============ sd-recfetch: the case sheet is opened per record and the page
   fetches that record itself; part_number is one of the fields it returns.
   The response is picked up in passing: fetch and XHR are wrapped once, and
   only a response that looks like one record (own part_number field, no
   records array, and not one of the dossier endpoints) is kept, latest one
   winning, because the sheet shows one record at a time. ================== */
var SX={last:null};
...
/* ============ sd-pnrow: the case sheet's sixteen rows carry a Part row that
   names the part ("AHRS"), but the part number appears nowhere on the page.
   The record endpoint returns part_number, so the sheet gets its own Part
   number row beside the Part row, and that row is the way into the part
   dossier. When the record has no part number, the row says so in the same
   words the sheet uses for its other absent fields, detected from those rows
   themselves. ============================================================= */
