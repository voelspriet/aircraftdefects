...
JavaScript (whole, interception removed, direct case fetch in its place):
...
What changed and why the fault is gone:

- **The wrappers are deleted.** `window.fetch` and `XMLHttpRequest.prototype.open` are no longer touched by this block at all — no clone, no shared state with another block's requests, no ordering hazard.
- **The block fetches the record itself.** When a case sheet is on screen (`sdSheets` finds the table with the Part and How-to-cite-it rows), the control number is read off the sheet: first from a row whose label names the control, failing that from the sheet's own text, skipping the row this block draws and the citation row. One `GET /z/api/case/<control>` goes out per control, cached in `SDPNC.cache`, one at a time per control.
- **`PartNumber` is the field read**, with `ComponentPartNumber` only as fallback. "not recorded" appears only when the response is in hand and both fields are empty — never while the request is in flight (the cell stays blank) and never on a failed request, so the FAA's number is never misrepresented as absent.
- **The row is the way in.** The filled value is a `code.cd.sd-pnlink` carrying `data-sd-part`, which the existing capture-phase click and keyboard handlers already route to `sdOpen("part", …)` — the part dossier, with `total 0` stated plainly instead of an empty table.
- Everything else — Phase B sentences, tab strip, rails, locate bar, operator/tail dossiers, phone layout, aria-labels — is untouched.