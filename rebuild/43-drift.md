Two notes before the blocks, since the brief asks for them explicitly:

- **The two 14px links.** `'SDR Desk lookup'` and `'Freefall: A Reckon…'` are links inside a sentence, so inflating them to 24px would break the sentence's line. I have not inflated the line height. Instead the block gives them `display: inline-block; line-height: 1; padding: 5px 2px; margin: -5px -2px;` — the hit box measures exactly 24px and the negative margins hand the padding straight back, so the line box the sentence sits in does not move. If your checker measures something other than the element box and this reads as cheating, say so and I will take those two out of the check.
- **The duplicate id.** The real fix is at the emit site: the strip builder should stop emitting its empty `<span id="iAim">`; the page block below is a workaround until that ships, and it runs before the instrument script binds, so nothing ever resolves the dropped node. It strips the *attribute*, not the node, so any content the second one had picked up is not destroyed — but it is unreachable by id, and `querySelectorAll('#iAim').length` is 1.

The CSS is one block that owns every name it uses. Each fault is marked where it is fixed; `#case-wrap`, `#case-box` and `.rr-sheet-lock` do not appear, and there is no `overflow` on `body`.
