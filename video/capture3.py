#!/usr/bin/env python3
"""Beats, recorded in real time. For every sentence the camera frames the
...
OUT = os.path.join(HERE, 'cap3'); os.makedirs(OUT, exist_ok=True)
...
class Shot:
    def __init__(self, pw, name):
        ...
        self.ctx = pw.chromium.launch().new_context(viewport={'width': RW, 'height': RH}, device_scale_factor=1,
                                                   record_video_dir=OUT, record_video_size={'width': RW, 'height': RH})
    ...
    def close(self):
        self.unzoom(0.6); self.hold(0.5)
        vid = self.pg.video.path(); self.ctx.close()
        shutil.move(vid, os.path.join(OUT, self.name + '.webm'))
        json.dump(self.log, open(os.path.join(OUT, self.name + '.json'), 'w'), indent=1); print('->', self.name)
